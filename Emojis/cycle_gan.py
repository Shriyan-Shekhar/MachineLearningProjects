...
def training_loop(dataloader_X, dataloader_Y, test_dataloader_X, test_dataloader_Y, device, opts):
    ...
    fixed_X = next(iter(test_iter_X))[0].to(device)
    fixed_Y = next(iter(test_iter_Y))[0].to(device)
    ...
    for iteration in range(1, opts.train_iters+1):
        ...
        D_X_loss = torch.sum(torch.square(D_X(images_X) - 1))/images_X.size(0)
        D_Y_loss = torch.sum(torch.square(D_Y(images_Y) - 1))/images_Y.size(0)
        ...
        D_X_loss = torch.sum(torch.square(D_X(fake_X)))/images_Y.size(0)

        # 4. Generate fake images that look like domain Y based on real images in domain X
        fake_Y = G_XtoY(images_X)

        # 5. Compute the loss for D_Y
        D_Y_loss = torch.sum (torch.square(D_Y(fake_Y)))/images_X.size(0)
        ...
        g_loss = torch.sum(torch.square(D_X(fake_X) - 1))/images_Y.size(0)

        # 3. Compute the cycle consistency loss (the reconstruction loss)
        cycle_consistency_loss = torch.mean(torch.square(images_Y - G_XtoY(G_YtoX(images_Y))))
        

        g_loss += cycle_consistency_loss
        G_YtoXCyclelosses.append(g_loss.item())
        ...
        g_loss = torch.sum(torch.square(D_Y(fake_Y) - 1))/images_X.size(0)

        # 3. Compute the cycle consistency loss (the reconstruction loss)
        cycle_consistency_loss = torch.mean(torch.square(images_X - G_YtoX(G_XtoY(images_X))))
        
        g_loss += cycle_consistency_loss
        G_XtoYCyclelosses.append(g_loss.item())
...
if __name__ == '__main__':
    ...
    axs.plot(epochs, G_YtoXCyclelosses)
    axs.plot(epochs, G_XtoYCyclelosses)
    axs.plot(epochs, D_real_losses) 
    axs.plot(epochs, D_fake_losses) 
    
    plt.xlabel("Iteration")
    plt.ylabel("Loss")
    plt.legend(["Generator Y->X Loss", "Generator x->Y Loss","Discriminator Real Loss","Discriminator Fake Loss"], loc='upper right')
    plt.show()
